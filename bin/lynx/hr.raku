  ...
  <tr>
    <td>
	  ...
	  <td>
		...
		.qq|</select>
		<a href=hr.raku?action=edit&id=$employee_id&db=employee&login=$form->{login}&path=$form->{path} target=_blank>?</a>
		</td>
	      </tr>
|;
...
|;
  ...
  foreach $ref (@{ $form->{transactions} }) {
    ...
    $column_data{invnumber} = "<td><a href=ap.raku?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{invnumber}&nbsp;</a></td>";
    $column_data{reference} = "<td><a href=gl.raku?action=edit&id=$ref->{glid}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{reference}&nbsp;</a></td>";
