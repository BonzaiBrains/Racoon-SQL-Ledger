...
|;
  ...
  for $accno (sort keys %{ $form->{CHK} }) {
    ...
    for $ref (@{ $form->{CHK}{$accno} }) {
      ...
      $column_data{source} = "<td><a href=rp.raku?action=list_payments&account=$account&source=$source&db=$arap&vc=$form->{vc}&$form->{vc}=$name$reportflds&path=$form->{path}&login=$form->{login}&title=$title target=_new>$ref->{source}&nbsp;$vd</a></td>";
