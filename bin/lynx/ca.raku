...
|;
  ...
  foreach $ca (@{ $form->{CA} }) {
    ...
    $href = "<a href=$ca->{module}.raku?path=$form->{path}&action=edit&id=$ca->{id}&login=$form->{login}&callback=$form->{callback}>$ca->{reference}</a>";
    ...
    if ($ca->{vc_id}) {
      $href = "<a href=ct.raku?path=$form->{path}&action=edit&id=$ca->{vc_id}&db=$ca->{db}&login=$form->{login}&callback=$form->{callback}>$ca->{description}</a>";
