...
|;
  ...
  foreach $ref (sort { $a->{accno} cmp $b->{accno} } @{ $form->{TB} }) {
    ...
    $href = qq|ca.raku?action=list_transactions|;
...
|;
    ...
    if ($vc_id != $ref->{vc_id}) {
      ...
      $column_data{vc} = qq|<td><a href=ct.raku?path=$form->{path}&login=$form->{login}&action=edit&id=$ref->{vc_id}&db=$form->{vc}&callback=$callback>$ref->{name}</a></td>|;
    ...
    $href = qq|$ref->{module}.raku?path=$form->{path}&action=edit&id=$ref->{id}&login=$form->{login}&callback=|.$form->escape($form->{callback});
...
|;
    ...
    $column_data{"$form->{vc}number"} = qq|<td>$ref->{"$form->{vc}number"}</td>|;
    $column_data{name} = qq|<td><a href=ct.raku?path=$form->{path}&login=$form->{login}&action=edit&id=$ref->{vc_id}&db=$form->{vc}&callback=$callback>$ref->{name}</a></td>|;
...
|;
	...
	if ($payment->{$form->{sort}} ne $sameitem) {
      ...
      $href = ($payment->{vcid}) ? "<a href=ct.raku?action=edit&id=$payment->{vcid}&db=$form->{vc}&login=$form->{login}&path=$form->{path}&callback=$callback>" : "";
	...
	if ($payment->{invoice}) {
      ...
      $href = "<a href=${module}.raku?action=edit&id=$payment->{trans_id}&login=$form->{login}&path=$form->{path}&callback=$callback>";
