...
|;
  ...
  for $i (1 .. $numrows) {
    ...
    if ($i < $numrows) {
      ...
      $column_data{itemhref} = qq|<td><a href="ic.raku?login=$form->{login}&path=$form->{path}&action=edit&id=$form->{"id_$i"}" target=_blank>?</a></td>|;
      
      $itemhistory = qq| <a href="ic.raku?action=history&login=$form->{login}&path=$form->{path}&pickvar=sellprice_$i&id=$form->{"id_$i"}" target=popup>?</a>|;
    } else {
