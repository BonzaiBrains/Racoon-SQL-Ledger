...
if (-f "$form->{path}/custom/arap.raku") {
    eval { require "$form->{path}/custom/arap.raku"; };
}
if (-f "$form->{path}/custom/$form->{login}/arap.raku") {
    eval { require "$form->{path}/custom/$form->{login}/arap.raku"; };
}
...
sub new_number {
  ...
  } elsif ($form->{script} eq 'gl.raku') {
    $invnumber = "reference";
    $numberfld = "glnumber";
  } elsif ($form->{script} eq 'hr.raku') {
