...
if (-f "$form->{path}/custom/io.raku") {
  eval { require "$form->{path}/custom/io.raku"; };
}
if (-f "$form->{path}/custom/$form->{login}/io.raku") {
  eval { require "$form->{path}/custom/$form->{login}/io.raku"; };
}
...
|;
    ...
    if ($i < $numrows) {
      $zero = "0";
      $itemhref = qq| <a href="ic.raku?login=$form->{login}&path=$form->{path}&action=edit&id=$form->{"id_$i"}" target=_blank>?</a>|;
      $itemhistory = qq| <a href="ic.raku?action=history&history=sales&login=$form->{login}&path=$form->{path}&pickvar=sellprice_$i&id=$form->{"id_$i"}" target=popup>?</a>|;
...
| if $form->{selectprojectnumber};
    ...
    if ($form->{type} !~ /_quotation/) {
      $orderxref = qq|
                <b>$orderxrefnumber</b>
		<input name="ordernumber_$i" value="$form->{"ordernumber_$i"}">&nbsp;<a href=oe.raku?action=lookup_order&ordnumber=|.$form->escape($form->{"ordernumber_$i"},1).qq|&vc=customer&type=sales_order&pickvar=ordernumber_$i&path=$form->{path}&login=$form->{login} target=popup>?</a>
                <b>$poxrefnumber</b>
		<input name="customerponumber_$i" value="$form->{"customerponumber_$i"}">&nbsp;<a href=oe.raku?action=lookup_order&ordnumber=|.$form->escape($form->{"customerponumber_$i"},1).qq|&vc=vendor&type=purchase_order&pickvar=customerponumber_$i&path=$form->{path}&login=$form->{login} target=popup>?</a>
|;
    ...
    if ($form->{vc} eq 'customer') {
      if ($form->{type} =~ /(invoice|_order|quotation)/ && $form->{type} !~ /credit/) {
        ...
        $costprice = qq|
          ...
          <a href="ct.raku?action=lookup_name&db=vendor&login=$form->{login}&path=$form->{path}&pickvar=costvendor_$i&pickid=costvendorid_$i&name=$name" target=popup> ?</a>
          <b>$costlabel</b>
          <input name="cost_$i" class=inputright size=10 value="|.$form->format_amount(\%myconfig, $form->{"cost_$i"}, $form->{precision}).qq|">&nbsp;<a href="ic.raku?action=history&history=purchases&login=$form->{login}&path=$form->{path}&pickvar=cost_$i&id=$form->{"id_$i"}" target=popup> ?</a>
|;
        $costprice .= qq|
                <b>$marginlabel</b>
                $margin
...
<form method=post action=ic.raku>
