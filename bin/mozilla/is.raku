...
sub generate {
  ...
  for (qw(notes intnotes department warehouse)) { delete $form->{$_} }
    ...
    if ($form->{currency} ne $form->{defaultcurrency}) {
      ...
      $exchangerate .= qq|
	      ...
	      <td nowrap><input name="exchangerate" class="inputright" size="10" value="$form->{exchangerate}">
	          <a href="am.raku?action=list_exchangerates&transdatefrom=$fdm&transdateto=$ldm&currency=$form->{currency}&login=$form->{login}&path=$form->{path}" target=_blank>?</a></td>|;
    }
...
|;
  ...
  $vcref = qq|<a href="ct.raku?action=edit&db=$form->{vc}&id=$form->{"$form->{vc}_id"}&login=$form->{login}&path=$form->{path}" target=_blank>?</a>|;
...
|;
  ...
  $form->hide_form(qw(id type printed emailed queued title vc discount creditlimit creditremaining tradediscount business closedto locked shipped oldtransdate oldduedate recurring defaultcurrency oldterms cdt precision order_id remittancevoucher reference_rows referenceurl oldwarehouse olddepartment company));
...
|;
  ...
  for (split / /, $form->{taxaccounts}) {
    ...
    if ($form->{taxincluded}) {
      ...
      if ($form->{"${_}_base"}) {
	$form->{"${_}_total"} = $form->round_amount($form->{"${_}_base"} * $form->{"${_}_rate"} / (1 + $form->{"${_}_rate"}), $form->{precision});
...
|;
    ...
    $column_data{datepaid} = qq|<td align=center nowrap><input name="discount_datepaid" size="11" class="date" value="$form->{"discount_datepaid"}">|.&js_calendar("main", "discount_datepaid").qq|</td>|;
