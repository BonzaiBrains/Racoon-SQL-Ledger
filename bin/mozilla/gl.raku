...
|;
  ...
  foreach $ref (@{ $form->{GL} }) {
    ...
    $ref->{reference} ||= "&nbsp;";
    $column_data{reference} = "<td><a href=$ref->{module}.raku?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{reference}</td>";
    ...
    if ($ref->{vc_id}) {
      $column_data{name} = "<td><a href=ct.raku?action=edit&id=$ref->{vc_id}&db=$ref->{db}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{name}</td>";
    }
