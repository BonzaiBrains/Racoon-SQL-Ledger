...
sub login {
  ...
  if (($errno = $user->login(\%$form, $userspath)) <= -1) {
    ...
    if ($errno == 4 && $form->{admin}) {
      ...
      $form->info("<p><a href=menu.raku?login=$form->{login}&path=$form->{path}&action=display&main=company_logo&js=$form->{js}&password=$form->{password}>".$locale->text('Continue')."</a>");
  ...
  $form->{callback} = "menu.raku?action=display";
