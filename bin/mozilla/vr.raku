...
|;
  ...
  foreach $ref (@{ $form->{transactions} }) {
    ...
    if ($form->{readonly}) {
      ...
      $column_data{vouchernumber} = qq|<td><a href=$module{$form->{batch}}.raku?action=edit&transdate=$form->{transdate}&type=$form->{type}&batch=$form->{batch}&batchid=$form->{batchid}&id=$ref->{id}&$form->{vc}_id=$ref->{"$form->{vc}_id"}&path=$form->{path}&login=$form->{login}&callback=$callback&batchdescription=$batchdescription>$ref->{vouchernumber} </a></td>|;
    }
...
sub add_voucher {
  ...
  $form->{callback} = "$module{$form->{batch}}.raku?action=$sub{$form->{batch}}";
