...
|;
  ...
  foreach $ref (@{ $form->{all_project} }) {
    ...
    if ($ref->{name}) {
      $column_data{name} = qq|<td><a href=ct.raku?action=edit&db=customer&id=$ref->{customer_id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{name}</td>|;
    }
    ...
    $column_data{partnumber} = qq|<td><a href=ic.raku?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{partnumber}</td>|;
...
# $locale->text('Edit Group')
  ...
  $preview = ($form->{image}) ? qq|<a href=$images/$myconfig{dbname}/$form->{image}> ?| : qq| <a href="ic.raku?action=upload_image&login=$form->{login}&path=$form->{path}" target=popup>?</a>|;
