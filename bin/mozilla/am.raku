...
|;
  ...
  foreach $ref (@{ $form->{ALL} }) {
	...
	$href = "$formname{$ref->{formname}}{$ref->{tablename}}{module}.raku?action=edit&id=$ref->{trans_id}";
	for (qw(path login)) {
	  $href .= "&$_=$form->{$_}";
	}
    ...
    if ($ref->{login} ne 'admin') {
      $href = "hr.raku?action=edit&db=employee&id=$ref->{employee_id}";
      for (qw(path login)) {
	$href .= "&$_=$form->{$_}";
      }
      $href .= "&callback=$callback";
...
|;
	...
	$column_data{nextdate} = qq|<td nowrap><input name="nextdate_$k" size=11 value="$ref->{nextdate}" title="$myconfig{dateformat}">|.&js_calendar("main", "nextdate_$k").qq|</td>|;
      ...
      $module = "$ref->{module}.raku";
      $type = "";
      
      if ($ref->{module} eq 'ar') {
	$module = "is.raku" if $ref->{invoice};
	$ref->{amount} /= $ref->{exchangerate};
	$column_data{name} = qq|<td><a href=ct.raku?path=$form->{path}&login=$form->{login}&action=edit&id=$ref->{name_id}&db=$ref->{vc}&callback=$callback>$ref->{name}</a></td>|;
      }
      if ($ref->{module} eq 'ap') {
	$module = "ir.raku" if $ref->{invoice};
	$ref->{amount} /= $ref->{exchangerate};
	$column_data{name} = qq|<td><a href=ct.raku?path=$form->{path}&login=$form->{login}&action=edit&id=$ref->{name_id}&db=$ref->{vc}&callback=$callback>$ref->{name}</a></td>|;
      }
      if ($ref->{module} eq 'oe') {
...
sub process_transactions {
  ...
  for (my $i = 1; $i <= $pt->{lastndx}; $i++) {
	...
	if ($pt->{arid} || $pt->{apid}) {
	  if ($pt->{arid}) {
	    $form->{script} = ($pt->{invoice}) ? "is.raku" : "ar.raku";
	    $form->{ARAP} = "AR";
	    $form->{module} = "ar";
	    $invfld = "sinumber";
	  } else {
	    $form->{script} = ($pt->{invoice}) ? "ir.raku" : "ap.raku";
	    $form->{ARAP} = "AP";
	    $form->{module} = "ap";
	    $invfld = "vinumber";
	  }
  ...
  $form->{callback} = "am.raku?action=recurring_transactions&path=$form->{path}&login=$form->{login}&header=$form->{header}";
  $form->redirect;
