...
if (-f "$form->{path}/custom/aa.raku") {
      eval { require "$form->{path}/custom/aa.raku"; };
}
if (-f "$form->{path}/custom/$form->{login}/aa.raku") {
      eval { require "$form->{path}/custom/$form->{login}/aa.raku"; };
}
...
|;
    ...
    $module = ($ref->{invoice}) ? ($form->{ARAP} eq 'AR') ? "is.raku" : "ir.raku" : $form->{script};
    $module = ($ref->{till}) ? "ps.raku" : $module;
