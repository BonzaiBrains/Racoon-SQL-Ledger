...
|;
  ...
  foreach $ref (@{ $form->{all_documents} }) {
	...
	$href="$module{$ref->{formname}}{script}.raku?action=edit&id=$ref->{trans_id}&login=$form->{login}&path=$form->{path}";
...
sub edit {
  ...
  if ($form->{formname}) {
    ...
    if ($module{$form->{formname}}{script}) {
      $href="$module{$form->{formname}}{script}.raku?action=edit&id=$form->{trans_id}&login=$form->{login}&path=$form->{path}";
