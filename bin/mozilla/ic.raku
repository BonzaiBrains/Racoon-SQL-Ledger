...
require "$form->{path}/io.raku";
require "$form->{path}/cm.raku";
require "$form->{path}/js.raku";
...
sub form_header {
  ...
  $sellprice = qq|
		...
		<td><input name=sellprice class="inputright" size=11 value=$form->{sellprice}> <a href=ic.raku?action=history&login=$form->{login}&path=$form->{path}&pickvar=sellprice&id=$form->{id} target=popup>?</a></td>
	      </tr>
|;
...
|.$form->hide_form(checkinventory);
  ...
  $preview = ($form->{image}) ? " <a href=$images/$myconfig{dbname}/$form->{image}> ?" : qq| <a href="ic.raku?action=upload_image&login=$form->{login}&path=$form->{path}" target=popup>?</a>|;
...
|;
  ...
  foreach $ref (@{ $form->{parts} }) {
    ...
    $column_data{invnumber} = ($ref->{module} ne 'oe') ? "<td><a href=$ref->{module}.raku?action=edit&type=invoice&id=$ref->{trans_id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{invnumber}&nbsp;</a></td>" : "<td>$ref->{invnumber}&nbsp;</td>";
    $column_data{ordnumber} = ($ref->{module} eq 'oe') ? "<td><a href=$ref->{module}.raku?action=edit&type=$ref->{type}&id=$ref->{trans_id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{ordnumber}&nbsp;</a></td>" : "<td>$ref->{ordnumber}&nbsp;</td>";
    $column_data{quonumber} = ($ref->{module} eq 'oe' && !$ref->{ordnumber}) ? "<td><a href=$ref->{module}.raku?action=edit&type=$ref->{type}&id=$ref->{trans_id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{quonumber}&nbsp;</a></td>" : "<td>$ref->{quonumber}&nbsp;</td>";

    $column_data{name} = "<td>$ref->{name}</td>";
    $column_data{transdate} = "<td>$ref->{transdate}</td>";

    if ($ref->{vc_id}) {
      $column_data{name} = qq|<td><a href=ct.raku?path=$form->{path}&login=$form->{login}&action=edit&id=$ref->{vc_id}&db=$ref->{vc}&callback=$callback>$ref->{name}</a></td>|;
    }
...
|;
  ...
  foreach $ref (@{ $form->{all_parts} }) {
    ...
    $column_data{ordnumber} = "<td><a href=oe.raku?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&type=sales_order&callback=$callback>$ref->{ordnumber}&nbsp;</a></td>";
    
    $ref->{description} =~ s/\r?\n/<br>/g;
    $column_data{reqdate} = "<td nowrap>$ref->{reqdate}</td>";
    $column_data{description} = "<td>$ref->{description}</td>";
    
    $column_data{partnumber} = qq|<td><a href=ic.raku?path=$form->{path}&login=$form->{login}&action=edit&id=$ref->{parts_id}&callback=$callback>$ref->{partnumber}</a></td>|;
    
    $column_data{name} = qq|<td><a href=ct.raku?path=$form->{path}&login=$form->{login}&action=edit&id=$ref->{"$form->{vc}_id"}&db=$form->{vc}&callback=$callback>$ref->{name}</a></td>|;
...
sub history {
  ...
  $mdl{ar} = 'is.raku';
  $mdl{ap} = 'ir.raku';
  for (qw(so po sq rfq)) { $mdl{$_} = 'oe.raku' }
...
|;
  ...
  foreach $ref (@{ $form->{all_inventory} }) {
    ...
    if ($sameid != $ref->{id}) {
      $column_data{partnumber} = qq|<td><input type=hidden name="id_$i" value=$ref->{id}><a href=ic.raku?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{partnumber}</td>|;
      $column_data{description} = "<td>$ref->{description}&nbsp;</td>";
...
|;
  ...
  foreach $ref (@{ $form->{inventory_transfer} }) {
    ...
    $column_data{partnumber} = qq|<td><a href=ic.raku?action=edit&id=$ref->{id}&login=$form->{login}&path=$form->{path}&callback=$callback>$ref->{partnumber}</a></td>|;
  ...
  foreach $ref (@{ $form->{all_inventory} }) {
    ...
    if ($sameid != $ref->{id}) {
      ...
      if ($ref->{assemblyitem}) {
        ...
        $column_data{assemblynumber} = qq|<td><input type=hidden name="id_$i" value=$ref->{id}><a href=ic.raku?action=edit&id=$ref->{id}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{partnumber}</td>|;
      }
      $column_data{description} = "<td>$ref->{description}&nbsp;</td>";
      $column_data{unit} = "<td>$ref->{unit}&nbsp;</td>";
      $column_data{partsgroup} = "<td>$ref->{partsgroup}&nbsp;</td>";
