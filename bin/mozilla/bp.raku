  ...
  for $i (1 .. $myform->{rowcount}) {
    ...
    if ($myform->{"ndx_$i"}) {
      ...
      if ($myform->{batch} eq 'queue') {
        ...
        if ($myform->{"module_$i"} eq 'oe.raku') {
          &order_links;
          &prepare_order;
          $form->{formname} = $myform->{type};
          $inv = ($form->{formname} =~ /_quotation/) ? 'quo' : 'ord';
        } elsif ($myform->{"module_$i"} eq 'jc.raku') {
          $form->{formname} = $myform->{type};
        ...
        if ($myform->{"module_$i"} ne 'jc.raku') {
...
|;
  ...
  foreach $ref (@{ $form->{SPOOL} }) {
    ...
    $column_data{name} = qq|<td><a href=ct.raku?action=edit&id=$ref->{vc_id}&db=$ref->{db}&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{name}</a></td>|;
    ...
    <input type=hidden name="reference_$i" value="$ref->{id}">|;
      ...
      $column_data{name} = qq|<td><a href=hr.raku?action=edit&id=$ref->{employee_id}&db=employee&path=$form->{path}&login=$form->{login}&callback=$callback>$ref->{name}</a></td>|;
    } else {
