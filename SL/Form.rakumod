...
return; # disable for now
  ...
  if ($countrycode) {
    $self->{helpref} = qq|<a href=am.raku?action=display_form&file=doc/help/$countrycode/$file&path=$self->{path}&login=$self->{login} target=_blank class=help>|;
  } else {
    $self->{helpref} = qq|<a href=am.raku?action=display_form&file=doc/help/$file&path=$self->{path}&login=$self->{login} target=_blank class=help>|;
  }
