...
\parbox{\textwidth}{%
  ...
  \parbox[b]{.2\textwidth}{
    \includegraphics[scale=0.3]{Racoon-SQL-Ledger}
  }\hfill
  \begin{tabular}[b]{rr@{}}
  Tel: & <%tel%>\\
  Fax: & <%fax%>
...
\begin{tabularx}{\textwidth}{@{}rlXlrlrrr@{}}
  \textbf{Pol.} & \textbf{Číslo} & \textbf{Popis} & & \textbf{Množ.} &
    \textbf{Jedn.} & \textbf{Cena} & \textbf{Zlava \%} & \textbf{Hodnota} \\ [0.5em]
...
\textbf{D O B R O P I S \hspace{0.2cm} O D \hspace{0.2cm} D O D A V A T E L A}

\vspace{1cm}

\begin{tabularx}{\textwidth}{*{8}{|X}|} \hline
  \textbf{Faktúra} & \textbf{Dátum} & \textbf{Splat.} & \textbf{Objednávka}
  & \textbf{Osoba}
  <%if shippingpoint%>
  & \textbf{Miesto dod.}
  <%end shippingpoint>
...
<%end taxincluded%>
<%foreach tax%>
 DIČ. <%taxnumber%> & <%taxdescription%> on <%taxbase%> @ <%taxrate%> \% & <%tax%> \\
<%end tax%>
<%if not taxincluded%>
 & Celkom & <%invtotal%> \\
<%end taxincluded%>
